...
version = 'v4 - Calibration Araya 2 - 100 100 100'
...
def scoring(row):
    ...
    elif row['norm_N_Cov'] > 5.0 and row['norm_N_Cov'] <= 9.5 and row['norm_RNaseP'] >1.1:
        return('PLOD')
    elif row['norm_N_Cov'] > 9.5 and row['norm_RNaseP'] >=1.0:
        return('N_Cov Patient Positive')
    elif row['norm_N_Cov'] > 9.5 and row['norm_RNaseP']<= 1.0:
        return('Control_N_Cov')
    elif row['norm_N_Cov'] <= 5.0 and row['norm_RNaseP'] <=1.59:
        return('No_Call')
    elif row['norm_N_Cov'] > 5.0 and row['norm_N_Cov'] <= 9.5 and row['norm_RNaseP'] <1.0:
        return'CTRL_PLOD'
    else:
        return('missing')

def void_detect_neg(row):
    if row['norm_N_Cov'] > 9.5:
        return('Detected')
    elif row['norm_N_Cov'] > 5.0 and row['norm_N_Cov'] <= 9.5:
        return('PLOD')
    else:
        return('negative')
def logit(row):
